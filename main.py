...
from model_ogb_whh import EdgePathNN
from model_ogb_GCN import GCN_model
    ...
    for step, batch in enumerate(loader):
        ...
        if batch.x.shape[0] == 1:
            ...
            if "classification" in task_type:
                ...
                loss = mse_reg_criterion(pred.to(torch.float32)[is_labeled],
                                         batch.y.to(torch.float32)[is_labeled].view(pred.size()))
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=2039,
    ...
    program_name = str(args.path_type) + f'_bs_{args.batch_size}' + '_lr_' + str(
        args.lr) + weight_decay + edge_flag + now
    ...
    if args.dataset in ["ogbg-molhiv", "peptides-functional", "BBBP", "Bace", "ClinTox", "Sider", "Tox21", "Baceselect_max"]:
        maximize = True
    else:
        maximize = False
    ...
    if args.dataset == 'Qm9':
        i = 0  # select task
        dataset.data.y = dataset.data.y[:, i]
        split_idx = dataset.get_idx_split(len(dataset.data.y), train_size=110000, valid_size=10000, seed=42)
        # train_loader, valid_loader, test_loader = dataset[split_idx['train']], dataset[split_idx['valid']], dataset[split_idx['test']]
        train_loader = DataLoader(dataset[split_idx['train']], batch_size=args.batch_size, shuffle=True)
        valid_loader = DataLoader(dataset[split_idx['valid']], batch_size=args.batch_size, shuffle=False)
        test_loader = DataLoader(dataset[split_idx['test']], batch_size=args.batch_size, shuffle=False)

    elif args.dataset in ["BBBP", "ClinTox", "Tox21", "Sider", "HIV", "Esol", "Freesolv", "Lipop", "Bace"]:
        split_idx = dataset.get_idx_split(len(dataset.data.y), train_size=math.floor(dataset.data.y.shape[0] * 0.8), valid_size=math.floor(dataset.data.y.shape[0] * 0.1), seed=20)
        train_loader = DataLoader(dataset[split_idx['train']], batch_size=args.batch_size, shuffle=True)
        valid_loader = DataLoader(dataset[split_idx['valid']], batch_size=args.batch_size, shuffle=False)
        test_loader = DataLoader(dataset[split_idx['test']], batch_size=args.batch_size, shuffle=False)
    ...
    # MolPath
    model = EdgePathNN(params['hidden_dim'], params['cutoff'], params['y'], dataset.num_tasks,
    ...
    for seed in range(n_seeds):
        ...
        if args.dataset in ["ZINC", "peptides-functional", "peptides-structural", "Esol", "Freesolv", "Lipop",
                            "Lipopselect_max", "Lipopselect_min"]:
        ...
        for epoch in pbar:
            ...
            if maximize:
                if valid_perf[dataset.eval_metric] >= early_stopper.val_acc:
                    test_perf_at_best, test_loss_at_best = eval(model, device, test_loader, evaluator,
                                                                dataset.task_type)
            else:
                if valid_perf[dataset.eval_metric] <= early_stopper.val_acc:
                    test_perf_at_best, test_loss_at_best = eval(model, device, test_loader, evaluator,
            ...
            pbar.set_description(
                f"{seed + 1}/{n_seeds}  Epoch {epoch + 1} Val loss {round(val_loss, 3)} Val perf {valid_perf[dataset.eval_metric]:.3f} Best Val Loss {early_stopper.val_loss:0.3f} Best Val Perf {early_stopper.val_acc:0.3f} Test Perf @ Best {test_perf_at_best[dataset.eval_metric]:0.3f}")
    ...
    with open(os.path.join("results", args.dataset, args.dataset +"_" + "results_" + program_name + ".pkl"), "wb") as f:
